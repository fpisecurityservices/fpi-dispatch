...
export function toEntry(row) {
  ...
  return {
    ...
    // Status of the linked incident (joined in by the API), if any.
    incident_status: row.incident_status ?? null,
...
export function toProtocol(row) {
  // dispatch.js expects protocols as {category, trigger, steps:[...]}
  return {
    id:       row.id,
    category: row.category,
...
export function toBol(row) {
  // dispatch.js expects watch orders as {id, text, ts, by}
  return {
    id:   row.id,
    text: row.text,
    ts:   row.created_at,
    by:   row.by_who || 'Dispatch',
  };
}
